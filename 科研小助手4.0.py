...
ResearchDirector = StreamingAgent(
    "研究总监",
    model_config_name="deepseek1_config",
        sys_prompt=(
           f"""
...
with msghub(
    ...
    announcement=Msg("system", "研究团队组建完成！现在开始3轮讨论，共同制定本科生科研方案。每轮讨论结束后，用户会提供反馈意见。", "system"),  # 一个问候消息
) as hub:
    ...
    # 系统提示本轮开始
    hub.broadcast(Msg("system", f"===== 第{round}轮讨论开始 =====", "system"))
    # 研究总监引导本轮讨论
    hub.broadcast(Msg("研究总监", f"第{round}轮讨论开始，研究总监将引导讨论。初始研究想法是：{x}。请各位专家基于前几轮讨论和用户反馈提出建设性意见。", "assistant"))
    ResearchDirector()
    # 其他专家依次发言
    LiteratureReviewer()
    MethodologyExpert()
    ...
    hub.delete(Critic)
    # 用户参与本轮讨论
    announce(Msg("user", f"第{round}轮讨论已结束，请提出您的意见（可对讨论内容进行补充、修改或提问）：", "user"))
    x = user_agent(x)
    # 研究总监回应用户反馈
    ResearchDirector()
    MethodologyExpert()
